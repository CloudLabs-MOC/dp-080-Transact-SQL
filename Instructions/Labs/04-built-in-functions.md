...
1. Run the query, and and after a few seconds, review the results, noting that the **YEAR** function has retrieved the year from the **SellStartDate** field.

1. Modify the query as follows to use some additional scalar functions that operate on *datetime* values.
...
1. Run the query and note that it returns the concatenated first and last name for each customer.
1. Replace the query with the following code to explore some more functions that manipulate string-based values.
...
1. Run the query and note that it returns the following data:
